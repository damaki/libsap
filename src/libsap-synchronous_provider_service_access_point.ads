...
is
   ...
   generic
      with procedure Build (Request : out Request_Type);
   procedure Build_And_Send_Request (Promise : in out Confirm_Promise)
   with Pre => Is_Null (Promise);
   --  Allocates a request, builds the request, then sends it to the Service
   --  Provider.
   --
   --  This procedure is used to send requests that may or may not require
   --  a confirmation.
   --
   ...
   --  future when it is ready. Otherwise, if the request does not expect a
   --  confirm primitive, then Promise is set to null.
   --
   --  This is a potentially blocking operation.

   generic
      with procedure Build (Request : out Request_Type);
   procedure Build_And_Send_Request_No_Confirm;
   --  Allocates a request, builds the request, then sends it to the Service
   --  Provider.
   --
   --  This procedure is used to send requests that never require a
   --  confirmation.
   --
   --  The postcondition of the Build generic formal procedure must contain:
   --  not Requires_Confirm (Request)
   --
   --  This is a potentially blocking operation.

   generic
      with procedure Build (Request : out Request_Type);
   procedure Build_And_Send_Request_With_Confirm
     (Promise : in out Confirm_Promise)
   with Pre => Is_Null (Promise), Post => not Is_Null (Promise);
   --  Allocates a request, builds the request, then sends it to the Service
   --  Provider.
   --
   --  This procedure is used to send requests that always require a
   --  confirmation.
   --
   --  The postcondition of the Build generic formal procedure must contain:
   --  Requires_Confirm (Request)
   --
   --  This is a potentially blocking operation.
